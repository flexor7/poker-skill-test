...
из пула в 100 вопросов на каждый раздел (всего 300). Каждое прохождение разное.

Тематический фокус — турниры (push/fold, ICM, bubble, final table, KO/PKO),
плюс базовые правила и универсальные концепции в лёгком разделе.
...
## Как добавлять/править вопросы

Открой `src/data/easy.js` (или `medium.js`, `hard.js`),
добавь объект в массив с уникальным `id` (формат `e101`, `m101`, ...).
Сборка подхватит автоматически.

Рандомизация выберет 10 из пула при каждом прохождении. Чтобы изменить число
вопросов за прохождение — поправь `QUESTIONS_PER_SECTION` в `src/data/index.js`.
